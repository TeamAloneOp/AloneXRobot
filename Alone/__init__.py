...
def get_user_list(__init__, key):
    with open("{}/Alone/{}".format(os.getcwd(), __init__), "r") as json_file:
        return json.load(json_file)[key]


# enable logging
FORMAT = "[ᴀʟᴏɴᴇ] %(message)s"
logging.basicConfig(
...
LOGGER = logging.getLogger("[ᴀʟᴏɴᴇ]")
LOGGER.info("ᴀʟᴏɴᴇ ɪs sᴛᴀʀᴛɪɴɢ. | ᴀɴ ᴀʟᴏɴᴇsʜᴇᴀᴠᴇɴ ᴘʀᴏᴊᴇᴄᴛ ᴘᴀʀᴛs. | ʟɪᴄᴇɴsᴇᴅ ᴜɴᴅᴇʀ ɢᴘʟᴠ3.")
...
SPAMWATCH_SUPPORT_CHAT = "@AlonesHeaven"
...
try:
    ...
    raise Exception(
        "[ᴀʟᴏɴᴇ ᴇʀʀᴏʀ]: Your ʀᴇᴅɪs ᴅᴀᴛᴀʙᴀsᴇ ɪs ɴᴏᴛ ᴀʟɪᴠᴇ, ᴘʟᴇᴀsᴇ 𝐂𝐡𝐞𝐜𝐤 ᴀɢᴀɪɴ."
    )
finally:
    REDIS.ping()
...
telegraph = Telegraph()
telegraph.create_account(short_name="Alone")
    ...
    if not isinstance(entity, Chat):
        ...
        try:
            ...
            for AloneX in apps:
                if AloneX != client:
                    try:
                        entity = await AloneX.get_chat(entity)
                    except (PeerIdInvalid, ChannelInvalid):
                        pass
                    else:
                        entity_client = AloneX
                        break
            else:
                entity = await AloneX.get_chat(entity)
                entity_client = AloneX
...
from Alone.modules.helper_funcs.handlers import CustomMessageHandler, CustomRegexHandler

tg.RegexHandler = CustomRegexHandler
tg.MessageHandler = CustomMessageHandler

from Alone.modules.helper_funcs.handlers import CustomCommandHandler
...
try:
    from Alone.antispam import antispam_cek_user, antispam_restrict_user, detect_user
